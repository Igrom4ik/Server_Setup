...
_Нажмите любую кнопку ниже для проверки:_"
...
SERVER_LABEL=$(jq -r '.telegram_server_label // "🌍 Сервер: $(hostname -I | awk \"{print \$1}\")"' "$CONFIG_FILE")
...
if [[ -f "\$CONFIG_FILE" ]]; then
    ...
    SERVER_LABEL="\${SERVER_LABEL:-"$SERVER_LABEL"}"
fi
